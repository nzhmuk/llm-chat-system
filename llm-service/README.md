...
This service exposes a simple HTTP API for interacting with a local LLM using Ollama (a Qwen2.5 instruct model by default).
...
| `--model`        | no       | `qwen2.5:3b` | Ollama model to pull and serve                                       |
| `--keep-alive`   | no       | `-1`         | How long Ollama keeps the model loaded (`-1` = forever; e.g. `30m`)  |
| `--num-predict`  | no       | `200`        | Max tokens per response (`-1` = unlimited)                           |
| `--think`        | no       | _(unset)_    | Reasoning for hybrid models like qwen3 (`false`/`true`); leave unset for non-thinking models |
...
| `OLLAMA_MODEL`       | `qwen2.5:3b`             | Model used for generation                                          |
| `OLLAMA_KEEP_ALIVE`  | `-1`                     | How long Ollama keeps the model loaded in memory (`-1` = forever)  |
| `OLLAMA_NUM_PREDICT` | `200`                    | Max tokens per response (`-1` = unlimited)                         |
| `OLLAMA_THINK`       | _(unset)_                | Reasoning toggle for hybrid models (qwen3); leave unset otherwise  |
| `LLM_API_KEY`        | _(generated)_            | Required. Clients must send `Authorization: Bearer <key>`          |

The automated installer writes these to `/etc/llm-api/llm-api.env` (loaded by
the systemd unit via `EnvironmentFile`) and prints the generated `LLM_API_KEY`
at the end — set that same value as `LLM_API_KEY` in the web app's `.env`. For
manual setup, copy `api/.env.example` to `api/.env` and adjust as needed.

## Authentication

All `/chat` endpoints require an API key sent as `Authorization: Bearer <key>`;
requests without a valid key get `401`. `/health` is left open for monitoring.
The installer generates the key and reuses the existing one on re-install (so
updates don't break the web app).
...
The installer tunes Ollama (not the FastAPI service) via a systemd drop-in at
`/etc/systemd/system/ollama.service.d/concurrency.conf`:

| Variable              | Value | Description                                      |
| --------------------- | ----- | ------------------------------------------------ |
| `OLLAMA_NUM_PARALLEL` | `2`   | Generations processed simultaneously per model   |
| `OLLAMA_MAX_QUEUE`    | `16`  | Requests allowed to wait before new ones are rejected |

On a CPU host, parallel generations share cores (each gets slower), so this is
kept modest: 2 active plus a queue comfortably serves ~5 concurrent users. Raise
`OLLAMA_NUM_PARALLEL` only if the host has spare CPU/RAM (each slot adds context
memory). Edit the drop-in, then `systemctl daemon-reload && systemctl restart ollama`.
